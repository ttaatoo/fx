...
import { ANTHROPIC_MODEL, wasmAnthropicEnv } from "./supergrok-fixture.mjs";
...
if (!apiKey) {
  console.error("Set ANTHROPIC_API_KEY to run the opt-in live Anthropic smoke test");
  process.exit(2);
}
...
const tracedFetch = async (url, init) => {
  if (init.method === "GET") throw new Error(`live catalog fetch is not a SuperGrok/Anthropic path: ${url}`);
  fetchCalls++;
...
const agent = await Promise.race([
  createFxAgent({
    ...
    fetch: tracedFetch,
    env: wasmAnthropicEnv({ ANTHROPIC_API_KEY: apiKey, FX_MODEL: ANTHROPIC_MODEL }),
  }),
...
try {
  ...
  if (responseStatus !== 200) throw new Error(`live Anthropic returned HTTP ${responseStatus}`);
  if (fetchCalls !== 1) throw new Error(`expected one live Anthropic fetch, got ${fetchCalls}`);
  ...
  console.log(`Anthropic HTTP ${responseStatus}; body chunks=${responseBodyChunks}; ACP text chunks=${chunks.length}`);
